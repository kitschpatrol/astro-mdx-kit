...
function parse(source: string, options: Parameters<typeof remarkMdxKitPlugin>[0]): Root {
	return unified().use(remarkParse).use(remarkMdxKitPlugin, options).parse(source) as Root
}

function transform(source: string, options: Parameters<typeof remarkMdxKitPlugin>[0]): Root {
...
describe('attributes option (T3 — end-to-end)', () => {
	...
	it('attaches kramdown attributes to the preceding paragraph when attributes is true', () => {
		...
		const hProperties = paragraph?.data?.hProperties as Record<string, unknown> | undefined
	...
	it('attaches kramdown attributes inline to images', () => {
		...
		visit(tree, 'image', (node) => {
			imageData = node.data?.hProperties as Record<string, unknown> | undefined
		})
...
describe('transform composition (T4 — end-to-end)', () => {
	it('runs directives + elements + captionImages + unwrapImages together', () => {
		...
		expect(callout?.attributes.some((a) => a.type === 'mdxJsxAttribute' && a.name === 'title')).toBe(
			true,
		)
